...
export class ShareChatDownloader extends DownloaderService {
  readonly platform: PlatformType = 'sharechat';
  ...
  public async analyze(url: string): Promise<MediaAnalysisResult> {
    try {
      ...
      for (const fmt of rawFormats) {
        ...
        const height = fmt.height || (fmt.format_note?.includes('720') ? 720 : 720);
      ...
      const maxAvailableQuality =
        sortedFormats.length > 0 ? sortedFormats[0].quality : '720p';
      ...
      return {
        ...
        maxAvailableQuality: '',
        formats: [],
        error: err.message || 'Unable to access this ShareChat video.',
      };
    }
  }
  ...
  public async executeDownload(
    ...
    const files = await YtDlpWrapper.downloadMedia(
      ...
      options.quality,
      (dlPercent) => {
        onProgress?.(15 + Math.floor(dlPercent * 0.5), 'Downloading media stream');
      }
    );
    ...
    if (files.length > 1 && videoFile && audioFile && videoFile !== audioFile) {
      onProgress?.(70, 'Merging ShareChat video and audio streams using FFmpeg');
